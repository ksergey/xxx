...
public:
  constexpr im_fixed_vector(im_fixed_vector const& other) : capacity_(other.capacity_), size_(other.size_) {
    data_ = std::make_unique_for_overwrite<T[]>(capacity_);
    std::copy_n(other.data(), other.size(), data_.get());
  }

  constexpr im_fixed_vector& operator=(im_fixed_vector const& other) {
    if (this != &other) {
      this->~im_fixed_vector();
      new (this) im_fixed_vector(other);
    }
    return *this;
  }

  constexpr im_fixed_vector(im_fixed_vector&& other) noexcept
      : data_(std::move(other.data_)), capacity_(std::exchange(other.capacity_, 0)),
        size_(std::exchange(other.size_, 0)) {}

  constexpr im_fixed_vector& operator=(im_fixed_vector&& other) noexcept {
    if (this != &other) {
      this->~im_fixed_vector();
      new (this) im_fixed_vector(std::move(other));
    }
    return *this;
  }

  constexpr im_fixed_vector() = default;

  constexpr im_fixed_vector(std::size_t capacity) : capacity_(capacity) {
    assert(capacity > 0);
